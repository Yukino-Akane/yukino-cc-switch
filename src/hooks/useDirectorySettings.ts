...
> = {
  ...
  yukino: { key: "yukino", defaultFolder: ".yukino" },
};
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("yukino"),
          ...
          computeDefaultConfigDir("yukino"),
        ]);
        ...
        setResolvedDirs({
          ...
          yukino: yukinoDir || defaultsRef.current.yukino,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        yukino: overrides?.yukino ?? defaultsRef.current.yukino,
      });
    },
    [],
  );
